...
if (!empty($_POST['content'])) {
	$allowed = '<div><p><span><br><a><img><h1><h2><h3><h4><ul>
	<ol><li><blockquote>';
...
if (empty($add_page_errors)) { 
	$q = "INSERT INTO pages (category_id, title, description,
    content) VALUES ($cat, '$t', '$d', '$c')";
	$r = mysqli_query ($dbc, $q);

	if (mysqli_affected_rows($dbc) == 1) { 

	echo '<div class = "alert alert-success"><h3>The page has
	been added!</h3></div>';
...
"utf-8">
<fieldset><legend>Fill out the form to add a page of content:
</legend>
<?php 
create_form_input('title', 'text', $add_page_errors); 
echo '<div class = "form-group"';
if(array_key_exists('category',$add_page_errors)) echo '
has-error';
	echo '<label> for="category" class="control-label">Category
	</label>
...
toolbar1:  "cut, copy, paste, |, undo, redo, removeformat, | hr ,|,
link, unlink, anchor, image, |, charmap, media, |, search, replace,|,
fullscreen",

toolbar2:  "bold, italic, underline, strikethrough ,|, alignleft,
aligncenter, alignright, alignjustify, |, formatselect, | ,bullist,
