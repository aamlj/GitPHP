...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (!empty($_POST['title'])) {
		$t = escape_data(strip_tags($_POST['description']),$dbc);
} else {
...
if (move_uploaded_file($file['tmp_name'], $dest)) {
 ...
 echo '<divclass="alert alert-sucess"><h3>The file has been uploaded!</h3></div>';
} else {
...
case 7:
case 8:
 $add_pdf_errors['pdf'] = 'The file could not be uploaded due to
a system error.';
break;
case 4:
default:
...
['file_name']);
 $tmp_name = escape_data ( $_SESSION['pdf']['temp_name'],$dbc);
...
$original = PDFS_DIR . $tmp_name;
$dest = PDFS_DIR . $tmp_name];
 rename($original, $dest);
 
 echo '<div class = "alert alert-success"><h3>The PDF has been 
 added!</h3></div>';
$_POST = array();
$_FILES = array();
...
<fieldset><legend>Fill out the form to add a PDF to the site:
</legend>
...
echo '"><label for="pdf" class="control-label">PDF</label>
<input type="file"name="pdf" id="pdf">'; 
if (array_key_exists('pdf', $add_pdf_errors)) {
 echo ' <span class="help-block">' . $add_pdf_errors)){
	 '</span>';
 }else{
 ...
 ?>
 <input type="submit" name="submit_button" value="Add This PDF" 
